...
public class TranspositionCipher {
        ...
        for (int i = 0; i < totalLength; i++) {
            ...
            if (i < plainText.length()) {
                ...
                if (plainText.charAt(i) == ' ')
                    matrix[row][colTemp] = '#';
                else matrix[row][colTemp] = plainText.charAt(i);
            } else {
                matrix[row][colTemp] = '-';
            }
        }
    ...
    public String doDecryption(final String cipherText) {
        final char[] encrypted = cipherText.toCharArray();
        // doProcessKey();
        final int row = cipherText.length();
        ...
        for (int i = 0; i < selectedKey.length(); i++) {
            ...
            for (int j = 0; j < row; j++) {
                tempCtr++;
                if (tempCtr < encrypted.length)
                    matrix[j][k] = encrypted[tempCtr];
            }
        }
